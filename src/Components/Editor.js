...
class Editor extends React.Component {
    ...
    trimVideo = () => {
        ...
        const result = this.state.timings.map((each) => {
            console.log(each,"Each")
            return Math.abs(each.start - each.end)
        })
        const sum = result.reduce(function(sum, number) {
            const updatedSum = sum + number;
            return updatedSum;
          }, 0);
        console.log(sum)
        if(sum > 60){
            alert("upload video should be one 1 min, trim more..!")
        }
        else if(sum <= 60){
            alert("Video Trimmed Successfully")
        }
    ...
    render = () => {
        return(
            <div className="wrapper">
                <video className="video" autoload="metadata" controls muted={this.state.isMuted} ref={this.playVideo} onClick={this.play_pause.bind(this)} >
                ...
                <div className="controls">
                    ...
                    <div>
                        ...
                        <button title="Delete grabber" className="trim-control margined" onClick={this.deleteGrabber}>Delete <FontAwesomeIcon icon={faGripLinesVertical} /></button>
                        <button title="Save changes" className="trim-control" onClick={this.trimVideo}>Trim</button>
                    </div>
                </div>
                ...
                {(this.state.imageUrl !== "") ? 
                    <div className={"marginVertical"}>
                        <img src={this.state.imageUrl} className={"thumbnail"} alt="image_url" />
                        <div className="controls">
                            <div className="player-controls">
